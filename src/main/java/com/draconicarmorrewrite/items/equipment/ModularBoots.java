...
public class ModularBoots extends ArmorItem implements IModularArmor, IDEEquipment, EquippedModelItem {
    private final TechLevel techLevel;

    public ModularBoots(TechProperties props) {
        super(ModularArmorMaterial.INSTANCE, Type.BOOTS, props);
    ...
    public boolean canEquip(ItemStack stack, LivingEntity livingEntity, String slotID) {
        if (!slotID.equals("feet") || !EquipmentManager.findItem(e -> e.getItem() instanceof ModularBoots, livingEntity).isEmpty()) {
            return false;
        }
    ...
    public ModuleHostImpl createHost(ItemStack stack) {
        ModuleHostImpl host = new ModuleHostImpl(techLevel, ModuleCfg.chestpieceWidth(techLevel), ModuleCfg.chestpieceHeight(techLevel), "boots", ModuleCfg.removeInvalidModules);
        host.addCategories(ModuleCategory.ARMOR, ModuleCategory.ARMOR_FEET, ModuleCategory.ALL, ModuleCategory.CHESTPIECE);
    ...
    public EquippedItemModel getExtendedModel(LivingEntity entity, ItemStack stack, @Nullable EquipmentSlot slot, HumanoidModel<?> parentModel, boolean slim) {
        ...
        if (model == null || model_on_armor == null) {
            model = new ModularArmorModel<>(techLevel, false, EquipmentSlot.FEET);
            model_on_armor = new ModularArmorModel<>(techLevel, true, EquipmentSlot.FEET);
        }
